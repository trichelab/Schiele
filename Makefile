...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "Schiele", "tests")); system.time(test_check("Schiele", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran Schiele_$(VERSION).tar.gz
	rm -rf Schiele.Rcheck/
